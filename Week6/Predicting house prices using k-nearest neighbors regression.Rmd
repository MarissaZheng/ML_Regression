...
Q: What is the Euclidean distance between the query house and the 10th house of the training set?  0.060
...
Q: : Among the first 10 training houses, which house is the closest to the query house? 6
...
rss <- apply((predicted_price - dt_validate$price)^2, 1, sum)
which.min(rss)
```

What is the RSS on the TEST data using the value of k found above? To be clear, sum over all houses in the TEST set. 
```{r}
predicted_price_on_test <- predict_output(k = 14, feature_train = feature_train, output_train = dt_train$price, feature_query = feature_test)
